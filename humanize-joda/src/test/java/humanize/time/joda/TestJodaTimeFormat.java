...
import static humanize.time.joda.FormatNames.FORMAT_JODA_PERIOD;
...
public class TestJodaTimeFormat
{
	...
	public void instance() throws ParseException
	{
		...
		Period period = new Period(0, 10244444);
		fmt = JodaTimeFormatProvider.factory().getFormat(FORMAT_JODA_PERIOD, null, Locale.ENGLISH);
		Assert.assertEquals(fmt.format(period), "2 hours, 50 minutes, 44 seconds and 444 milliseconds");
		Assert.assertEquals(fmt.parseObject("2 hours, 50 minutes, 44 seconds and 444 milliseconds"), period);
