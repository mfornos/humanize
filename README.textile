...
Provided in two variants: *humanize-slim* and *humanize-icu*. Humanize-slim only depends on guava and standard Java APIs while humanize-icu provides a concise facade for access to the "International Components for Unicode":http://icu-project.org/ (ICU) Java APIs.
...
bc.. <humanize.version>1.1.5</humanize.version>
...
| naturalTime | Computes both past and future relative dates with optional precision. | '2 days from now', '3 decades ago', '3 minutes from now', 'moments ago', etc. | ✔ | ✔ |
...
p. * This will be replaced by the new Units of Measurement API: __JSR 363__
p. See "Units of Measurement site":http://unitsofmeasurement.github.io/ for more information
